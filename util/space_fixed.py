...
def find_uniform(number):
	factor = 5
	number = int(number)
	if(number <= 50): step = int(1*factor)
	elif(number <= 200): step = int(4*factor)
	elif(number <= 500): step = int(10*factor)
	elif(number <= 1000): step = int(20*factor)
	else: step = int(20*factor) 
	divisor_list = [1]
	divisor = 0
